...
const DentistSchedule = () => {
  ...
  // To track the last shown toast to avoid duplicates
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      try {
        ...
        setAvailableSlots([]);
        // Avoid multiple error toasts for fetchSlots, just set error silently here
      } finally {
        setLoading(false);
      }
    };
    ...
    const fetchSchedule = async () => {
      ...
      try {
        ...
        if (
          ...
          setAvailableTime(response.data.schedule.availableTime);
        } else {
          setAvailableTime([]);
        }
      } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    setError("");
    lastToastId.current = null; // reset last toast for this submit
    ...
    try {
      ...
      // Success toast only if not shown already
      if (!lastToastId.current) {
        lastToastId.current = toast.success(
      ...
      setAvailableTime(generatedSlots);
    } catch (err) {
      const message = err.response?.data?.message || "Error updating schedule.";
      setError(message);
      if (!lastToastId.current) {
        lastToastId.current = toast.error(message);
      }
    } finally {
      setLoading(false);
    }
        ...
        <div className="w-full max-w-4xl p-8 bg-white rounded-2xl shadow-2xl border border-gray-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-4 flex justify-center">
              <button
                type="submit"
                className={`w-full md:w-1/3 py-3 text-base font-semibold text-white rounded-xl transition shadow-md flex justify-center items-center ${
                  loading
                    ? "bg-teal-400 cursor-not-allowed"
                    : "bg-teal-500 hover:bg-teal-600"
                }`}
